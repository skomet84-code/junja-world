...
function focusInput(input:HTMLInputElement){try{input.focus({preventScroll:true});}catch{input.focus();}}
...
function repairAccountGate(){
  ...
  g.removeAttribute('inert');g.style.pointerEvents='auto';g.style.touchAction='manipulation';
  ['#jw26-id','#jw26-password'].forEach(selector=>{
    ...
    if(input.dataset.jw286Bound==='1')return;input.dataset.jw286Bound='1';
    input.addEventListener('pointerdown',e=>{e.stopPropagation();window.setTimeout(()=>focusInput(input),0);});
...
function repairCharacterEntry(){
  ...
  if(!a||!open)return;
  a.removeAttribute('inert');a.style.pointerEvents='auto';a.style.touchAction='manipulation';
  ...
  if(name){
    ...
    name.style.pointerEvents='auto';name.style.touchAction='manipulation';
    if(name.dataset.jw286Bound!=='1'){
      name.dataset.jw286Bound='1';
      name.addEventListener('pointerdown',e=>{e.stopPropagation();window.setTimeout(()=>focusInput(name),0);});
      name.addEventListener('touchstart',e=>e.stopPropagation(),{passive:true});
      name.addEventListener('click',e=>{e.stopPropagation();focusInput(name);});
    }
  }
  document.querySelectorAll<HTMLButtonElement>('#auth-layer .class-card,#auth-layer #enter-game').forEach(button=>{
    button.disabled=false;button.removeAttribute('disabled');button.tabIndex=0;button.style.pointerEvents='auto';button.style.touchAction='manipulation';
  });
}
...
function boot(){
  ...
  if(g)new MutationObserver(repair).observe(g,{attributes:true,attributeFilter:['class','style']});
  if(a)new MutationObserver(repair).observe(a,{attributes:true,attributeFilter:['class','style']});
